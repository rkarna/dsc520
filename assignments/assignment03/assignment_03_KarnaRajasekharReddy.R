...
## `height` vs. `earn`
ggplot(heights_df, aes(x=height, y=earn)) + geom_smooth(stat = 'smooth', color = 'Red', method = 'gam', , formula = y ~ s(x, bs = "cs")) + geom_point()
## `age` vs. `earn`
ggplot(heights_df, aes(x=age, y=earn)) + geom_point() + geom_smooth(stat = 'smooth', color = 'Red', method = 'gam', , formula = y ~ s(x, bs = "cs"))
## `ed` vs. `earn`
ggplot(heights_df, aes(x=ed, y=earn)) + geom_point() + geom_smooth(stat = 'smooth', color = 'Red', method = 'gam', , formula = y ~ s(x, bs = "cs"))

## Create a scatterplot of `height`` vs. `earn`.  Use `sex` as the `col` (color) attribute
ggplot(heights_df, aes(x=height, y=earn, col=sex)) + geom_point(aes(color="red"))
...
ggplot(heights_df, aes(x=height, y=earn, col=height,earn)) + geom_point(aes(color="red")) + ggtitle("Height vs. Earnings") + xlab("Height (Inches)") + ylab("Earnings (Dollars)")

# https://ggplot2.tidyverse.org/reference/geom_histogram.html
## Create a histogram of the `earn` variable using `geom_histogram()`
ggplot(heights_df, aes(earn)) + geom_histogram(bins = 30, color = "white")

## Create a histogram of the `earn` variable using `geom_histogram()`
## Use 10 bins
ggplot(heights_df, aes(earn)) + geom_histogram(bins = 10, color = "white")
