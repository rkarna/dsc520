...
ggplot(heights_df, aes(sex)) + geom_bar(color = 'Red')

## Using `geom_bar()` plot a bar chart of the number of records for each race
ggplot(heights_df, aes(race)) + geom_bar(color="red")

## Create a horizontal bar chart by adding `coord_flip()` to the previous plot
ggplot(heights_df, aes(race)) + geom_bar(color="red") + coord_flip(xlim=c(-20, 10))
